...
pub(crate) async fn run_article(url: &str, date_override: &str, use_ai_summary: bool) -> Result<(), String> {
    ...
    let (title, body_text) = if html_body.split_whitespace().count() < 200 {
        tracing::debug!("[Article] Thin content ({} words) — trying Jina Reader…", html_body.split_whitespace().count());
        match fetch_via_jina(url).await {
            Some((jina_title, jina_body)) => {
                tracing::debug!("[Article] Jina Reader: {} words", jina_body.split_whitespace().count());
                let title = if jina_title.is_empty() { html_title } else { jina_title };
                (title, jina_body)
            }
            None => {
                tracing::debug!("[Article] Jina Reader failed — using direct fetch content");
    ...
    tracing::debug!("[Article] Title: {title:?}");
    tracing::debug!("[Article] Body text ({} chars)", body_text.len());
    ...
    let description = if use_ai_summary && hf_api_token().await.is_some() {
        ...
        tracing::debug!("[Article] Summary: {summary_result:?}");
