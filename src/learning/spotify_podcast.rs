...
async fn fetch_via_jina(episode_url: &str) -> Option<JinaMeta> {
    ...
    tracing::debug!("[Spotify] Calling Jina: {jina_url}");
    let body = http::get(&jina_url, &[], 30_000).await.ok()?;
    tracing::debug!("[Spotify] Jina response length: {} chars", body.len());
    ...
    tracing::debug!("[Spotify] Jina title: {title:?}");
    tracing::debug!("[Spotify] Jina description ({} chars): {:?}",
...
async fn fetch_episode_meta(episode_id: &str) -> Option<SpotifyEpisodeMeta> {
    ...
    let embed_html = http_get(&embed_url).await;
    let page_html = http_get(&page_url).await;
    ...
    let (title_from_ld, description, date_from_ld) = parse_json_ld(page_html.as_deref().unwrap_or(""));
    ...
    let jina = if need_jina {
        tracing::debug!("[Spotify] Falling back to Jina for missing/corrupt metadata…");
    ...
    Some(SpotifyEpisodeMeta { title, show_name, duration_ms, release_date, description })
}
...
pub(crate) async fn run_spotify_podcast(url: &str, date_override: &str, use_ai_summary: bool) -> Result<String, String> {
    ...
    tracing::debug!("[Spotify] Title: {title:?}");
    tracing::debug!("[Spotify] Duration: {} ms, Date: {:?}", meta.duration_ms, meta.release_date);
    ...
    let description = if meta.description.len() > 1000 {
        if use_ai_summary && hf_api_token().await.is_some() {
            let summary_result = summarize_with_bart(&meta.description).await;
            tracing::debug!("[Spotify] Summary: {summary_result:?}");
