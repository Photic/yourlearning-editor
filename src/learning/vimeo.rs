...
async fn fetch_oembed(url: &str) -> Option<VimeoMeta> {
    ...
    let text = http::get(&oembed_url, &[("User-Agent", "Mozilla/5.0 (compatible; yourlearning-editor)")], 15_000)
        .await
        .ok()?;
    let json: serde_json::Value = serde_json::from_str(&text).ok()?;

    tracing::debug!("[Vimeo] oEmbed response: title={:?} duration={:?}",
...
pub(crate) async fn run_vimeo(url: &str, date_override: &str, use_ai_summary: bool) -> Result<String, String> {
    tracing::debug!("[Vimeo] Fetching metadata for {url}");
    ...
    tracing::debug!("[Vimeo] Title: {title:?}, Duration: {}s", meta.duration_secs);
    ...
    let description = if !meta.description.is_empty() && use_ai_summary && hf_api_token().await.is_some() {
        let summary_result = summarize_with_bart(&meta.description).await;
        tracing::debug!("[Vimeo] Summary: {summary_result:?}");
